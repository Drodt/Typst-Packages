...
#let outline-slide(
  ...
  let body = {
    ...
    let sel = if show-sections and show-focus {
      focus-sel.or(section-sel)
    } else if show-sections {
      section-sel
    } else if show-focus {
      focus-sel
    } else {
      panic("Need to show sections or focus slides")
    }
...
///
/// ```typst
/// #title-slide(title: "Override Title", logo: image("../assets/tuda_logo.svg", width: 60%))
/// ```
///
...
/// ```typst
/// #hero(
/// image("../assets/tudo_logo.svg", height: 70%),
/// title: "Hero",
/// subtitle: "Subtitle",
/// )
/// ```
...
#let pbox(
  ...
  content-inset: .5em,
  header-inset: .5em,
  title-color: black.transparentize(10%),
  ..args,
) = {
  // check color types
  assert(
  ...
  let header = box(
    ...
    inset: header-inset,
    stroke: (right: border-width + header-color),
    {
      strong(delta: 200, text(fill: title-color, title))
    },
  )
  ...
  let content-box(content) = block(
    ...
    radius: (
      top-left: 0pt,
      bottom-left: 0pt,
      top-right: if title == none { radius } else { 0pt },
      rest: radius,
    ),
    content,
  )
...
#let alertbox = pbox.with(
  accent-color: red.A,
  header-color: red.A.lighten(65%),
  border-color: red.A.lighten(50%),
)
