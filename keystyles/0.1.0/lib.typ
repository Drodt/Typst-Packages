...
#let prooftree = prooftrees.tree.with(vertical_spacing: 6pt)

#let nary(n, name, concl, cond: none, def: false) = {
  let sp = 4pt
  let lbl = none
  if name != none {
    if def {
      lbl = [#smallcaps(name) #label("dl-" + name)]
    } else {
      lbl = smallcaps(name)
    }
  }
  prooftrees.nary(
    ...
    concl, 
    left_label: lbl,
    left_label_spacing: sp,
    right_label: cond,
    right_label_spacing: sp,
  )
}

#let dl-ref(name) = link(label("dl-" + name))[Rule~#smallcaps(name)]

#let axi(body) = nary(0, none, body)
#let todo-axi = axi[_TODO_]
#let uni(name, body, cond: none, def: false) = nary(1, name, body, cond: cond, def: def)
#let bin(name, body, cond: none, def: false) = nary(2, name, body, cond: cond, def: def)

#let closeBranch = axi(sym.star)

#let seqRule(name, ..pre, concl, cond: none) = {
	let pres = pre.pos()
	prooftree(
		..pres.map(p => axi(p)),
		nary(pres.len(), name, concl, def: true, cond: cond)
	)
}
